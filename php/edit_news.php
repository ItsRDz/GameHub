...
if (isset($_POST['submitted'])) {
	...
	if(isset($_POST['news_text'])){
		$desc = mysqli_real_escape_string($dbc, trim($_POST['news_text']));
	}
	...
	if (empty($errors)) {
		...
		// Query
		$q = "UPDATE news SET news_title = '$title', news_text = '$desc', genre = '$genre', platform = '$platform', image = '$image' WHERE news_id = $edit_id";
...
// Make the query:
$q = "SELECT news_id, news_title FROM news ORDER BY news_title ASC";
$result = mysqli_query($dbc, $q); // Run the query.

$zzz = @\mysqli_query($dbc, "SELECT * FROM news WHERE news_id = $edit_id");
...
<form class="form-signin" action="" method="POST">
	<h3>News Title:</h3>
	<input class="form-control" name="title" value="<?php echo $row_rev2['news_title']; ?>" />
	<br>
	
	<h3>News Text:</h3>
	<textarea class="form-control" name="news_text" rows="5" cols="40"><?php echo $row_rev2['news_text']; ?></textarea>
	<br>
